...
export default function Notes() {
  return (
    <section id="notes" className="py-32 bg-surface">
      <div className="max-w-5xl mx-auto px-6">

        <FadeIn>
          <p className="text-xs tracking-[0.2em] uppercase text-accent font-sans mb-6">Notes</p>
            ...
            <FadeIn key={i} delay={i * 0.08}>
              <article className="py-9 first:pt-0 border-b border-white/8 last:border-b-0 group">
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-xs font-sans text-foreground/30">{note.date}</span>
                  <span className="text-xs font-sans text-accent/60 border border-accent/25 px-2 py-0.5">{note.tag}</span>
                </div>

                <h3 className="font-serif text-xl font-bold text-foreground leading-snug mb-3 group-hover:text-accent transition-colors duration-200">
                  {note.href ? (
                    <a href={note.href} className="hover:text-accent transition-colors duration-200">{note.title}</a>
                  ) : note.title}
                </h3>

                <p className="text-sm font-sans font-light text-foreground/45 leading-relaxed mb-4">{note.excerpt}</p>

                {note.href ? (
                  <a href={note.href} className="inline-flex items-center gap-1.5 text-xs font-sans text-accent hover:text-foreground transition-colors duration-200">
